...
// If the checkout is successful, we'll get a confirmation.
const result = await response.json();

// Clear the local cart state
setCartItems([]);
setIsCartOpen(false); // Close the cart modal
showFlashMessage("Thank you for your purchase! Your order is being processed.", "success");

} catch (e) {
console.error("Oh no, an error during checkout:", e);
showFlashMessage(`Checkout failed: ${e.message}`, 'error');
}
};
// NEW FUNCTION: Closes the checkout modal.
...
className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-300 ease-in-out shadow-md"
>
Clear Cart
</button>
...
className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-300 ease-in-out shadow-md"
>
Checkout
</button>
</div>
...
<div className={`fixed inset-0 z-50 transition-opacity duration-300 ${isCheckoutModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
{/* Backdrop overlay */}
...
<h3 className="text-2xl font-bold text-gray-800 mb-4">Proceeding to Checkout</h3>
